...
function tinyaiex_gpt_utf8_encode(string $str): string 
{
    $str .= $str;
    $len = \strlen($str);
...
function tinyaiex_gpt_encode($text) 
{
    $bpe_tokens = array();
    if(empty($text))
    {
    ...
    foreach($bpe_merges_temp as $bmt)
    {
        ...
        $split_bmt = array_filter($split_bmt, 'tinyaiex_gpt_my_filter');
        if(count($split_bmt) > 0)
        {
            $bpe_merges[] = $split_bmt;
        }
    }
    $bpe_ranks = tinyaiex_gpt_dictZip($bpe_merges, range(0, count($bpe_merges) - 1));
    ...
    foreach($matches[0] as $token)
    {
        $new_tokens = array();
        $chars = array();
        $token = tinyaiex_gpt_utf8_encode($token);
        ...
        {
            if(isset($byte_encoder[tinyaiex_gpt_unichr($char)]))
            {
                $result_word .= $byte_encoder[tinyaiex_gpt_unichr($char)];
            }
        }
        $new_tokens_bpe = tinyaiex_gpt_bpe($result_word, $bpe_ranks, $cache);
...
function tinyaiex_gpt_decode($tokens) 
{
    $rencoder = file_get_contents(dirname(__FILE__) . "/encoder.json");
    $encoder = json_decode($rencoder, true);
    if(empty($encoder))
...
function tinyaiex_gpt_my_filter($var)
{
    return ($var !== NULL && $var !== FALSE && $var !== '');
}

function tinyaiex_gpt_unichr($c) 
{
...
function tinyaiex_gpt_dictZip($x, $y)
{
    $result = array();
    $cnt = 0;
    foreach($x as $i)
...
function tinyaiex_gpt_get_pairs($word) 
{
    $pairs = array();
    $prev_char = $word[0];
...
function tinyaiex_gpt_split($str, $len = 1) 
{
    $arr		= [];
    if(function_exists('mb_strlen'))
    {
...
function tinyaiex_gpt_bpe($token, $bpe_ranks, &$cache)
{
    if(array_key_exists($token, $cache))
    {
        return $cache[$token];
    }
    $word = tinyaiex_gpt_split($token);
    $init_len = count($word);
    $pairs = tinyaiex_gpt_get_pairs($word);
    if(!$pairs)
    {
        return $token;
    }
    while (true) 
    {
        ...
        while ($i < count($word)) 
        {
            $j = tinyaiex_gpt_indexOf($word, $first, $i);
            if ($j === -1) 
            {
...
function tinyaiex_gpt_indexOf($arrax, $searchElement, $fromIndex)
{
    $index = 0;
